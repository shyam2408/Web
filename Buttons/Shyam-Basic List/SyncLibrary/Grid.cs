...
namespace SyncLibrary
{
    public static class Grid<Type>
    {
        public static void PrintTables(CustomList<Type> list)
        {
            ...
            System.Console.WriteLine(new string('-',properties.Length*18));//printing the upper line of the table
            foreach(PropertyInfo property in properties)
            {
                System.Console.Write($"|{property.Name,-17}");//printing the table header
            }
            ...
            foreach(var info in list)
            {
                ...
                foreach(PropertyInfo prop in properties)
                {
                    ...
                    if(prop.CanRead)
                    {
                        ...
                        {
                            //handling DateTime
                            System.Console.Write($"|{((DateTime)prop.GetValue(info)).ToString("dd/MM/yyyy"),-16}|");
                        }
                        else//for all the other types
                        {
                            System.Console.Write($"|{prop.GetValue(info),-16}|");
                        }
                    }
                }
                System.Console.Write("\n");
